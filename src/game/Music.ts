/**
 * Music system for Lockdown using Web Audio API.
 * Generates tense, institutional chiptune background music.
 */
...
class MusicSystem {
  ...
  private playNote(freq: number, startTime: number, duration: number, vol: number = 1): void {
    ...
    // Square wave for harsh, institutional feel
    osc.type = 'square';
  ...
  // Gameplay track - tense institution, order and control
  private getGameplayNotes(): Note[] {
    ...
    // Institutional tension melody (8 bars)
    const melody = [
      // Bar 1-2: Morning count
      { note: 'D3', dur: quarter },
      { note: 'D3', dur: eighth },
      { note: 'F3', dur: eighth },
      { note: 'A3', dur: half },
  ...
  // Menu track - institutional calm before the storm
  ...
  // Game over track - lockdown failed
